  ...
  useEffect(() => {
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (isResizing.current) {
        updateSize({
          width: Math.min(Math.max(400, resizeStart.current.w + e.clientX - resizeStart.current.x), window.innerWidth - posRef.current.x),
          height: Math.min(Math.max(280, resizeStart.current.h + e.clientY - resizeStart.current.y), window.innerHeight - TASKBAR_H - posRef.current.y),
        });
      }
    };
    const onMouseUp = () => {
